    ...
    <div className="min-h-screen bg-slate-100 p-2 md:p-6 font-sans text-gray-800">
      <div className="max-w-7xl mx-auto bg-white shadow-2xl rounded-3xl border-t-8 border-blue-900 overflow-hidden relative">
        ...
        <div className="p-4 md:p-8">
          {/* ইনপুট ফরম - পিডিএফ অনুযায়ী প্রতিটি ফিল্ড */}
          <form onSubmit={handleSubmit} className={`p-6 md:p-10 rounded-3xl border-2 shadow-inner transition-all ${isEditing ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-100'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               ...
               <input type="text" name="eduHSC" value={formData.eduHSC} placeholder="HSC (জিপিএ/সাল/প্রতিষ্ঠান)" onChange={handleChange} className="p-3 border rounded-xl bg-white" />
            ...
            <div className="flex gap-4 mt-8">
                <button type="submit" className={`flex-grow py-4 rounded-2xl text-white font-bold shadow-xl uppercase transition-all duration-300 ${isEditing ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-900 hover:bg-black'}`}>
              ...
              <tbody className="divide-y divide-gray-100">
                {teachers.map((t) => (
                  <tr key={t._id} className="hover:bg-slate-50 transition border-b">
                    ...
                    <td className="p-5 flex flex-wrap justify-center gap-2 mt-2">
        ...
        {/* --- বিস্তারিত সলিড হোয়াইট মডাল (পিডিএফ-এর সব তথ্যসহ) --- */}
        {selectedTeacher && (
           <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 10000000 }}>
              ...
              <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-t-8 border-blue-900 relative z-[10000001]">
                  ...
                  <div className="p-8 md:p-10 space-y-8 bg-white">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 border-b pb-8 text-sm md:text-base">
                      <div className="space-y-3 text-gray-800">
                        <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1 tracking-widest">১. ব্যক্তিগত তথ্য</h3>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>নাম (বাংলা):</strong> <span>{selectedTeacher.nameBn || selectedTeacher.nameBN}</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>পিতার নাম:</strong> <span>{selectedTeacher.fatherName || "নেই"}</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>মাতার নাম:</strong> <span>{selectedTeacher.motherName || "নেই"}</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>এনআইডি:</strong> <span>{selectedTeacher.nid || "নেই"}</span></p>
                        <p className="text-xs bg-blue-50 p-3 rounded-lg mt-2"><strong>ঠিকানা:</strong> {selectedTeacher.presentAddr || selectedTeacher.address || "প্রদত্ত নয়"}</p>
                      </div>
                      <div className="space-y-3 text-gray-800">
                        <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1 tracking-widest">২. পেশাগত তথ্য</h3>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>পদবী:</strong> <span>{selectedTeacher.designation}</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>বিভাগ:</strong> <span>{selectedTeacher.department || "নেই"}</span></p>
                        <p className="flex justify-between py-1 text-green-700 font-bold border-b border-dotted"><strong>মূল বেতন:</strong> <span>{selectedTeacher.basicPay || 0} ৳</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>যোগদান তারিখ:</strong> <span>{selectedTeacher.firstJoinDate || "নেই"}</span></p>
                        <p className="flex justify-between border-b border-dotted py-1"><strong>PRL তারিখ:</strong> <span>{selectedTeacher.prlDate || "নেই"}</span></p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       <div className="space-y-3 text-gray-800">
                          <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1 tracking-widest">৩. শিক্ষাগত যোগ্যতা</h3>
                          <p><strong>SSC যোগ্যতা:</strong> <br/> <span className="text-gray-600 italic text-sm">{selectedTeacher.eduSSC || "নেই"}</span></p>
                          <p><strong>স্নাতক যোগ্যতা:</strong> <br/> <span className="text-gray-600 italic text-sm">{selectedTeacher.eduGrad || "নেই"}</span></p>
                       </div>
                       <div className="space-y-3 text-gray-800">
                          <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1 tracking-widest">৪. ব্যাংক ও আয়কর</h3>
                          <p className="flex justify-between border-b border-dotted py-1"><strong>ব্যাংক একাউন্ট:</strong> <span>{selectedTeacher.bankAcc || "নেই"}</span></p>
                          <p className="flex justify-between border-b border-dotted py-1"><strong>রাউটিং নম্বর:</strong> <span>{selectedTeacher.routingNumber || "নেই"}</span></p>
                          <p className="flex justify-between border-b border-dotted py-1"><strong>আয়কর কর্তন:</strong> <span>{selectedTeacher.incomeTax || "নেই"} ৳</span></p>
                       </div>
                    </div>
                  </div>

                  <div className="p-8 bg-gray-100 flex justify-center">
                    <button onClick={() => setSelectedTeacher(null)} className="bg-blue-900 text-white py-3 px-20 rounded-2xl font-bold shadow-lg hover:bg-black transition-all uppercase tracking-widest">বন্ধ করুন</button>
                  </div>
              </div>
           </div>
        )}
