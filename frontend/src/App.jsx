...
const TeacherDatabase = () => {
  ...
  return (
    <>
      <div className="min-h-screen bg-slate-100 p-2 md:p-6 font-sans text-gray-800">
        <div className="max-w-7xl mx-auto bg-white shadow-2xl rounded-3xl border-t-8 border-blue-900 overflow-hidden">
          ...
          <div className="bg-blue-900 text-white py-10 px-6 text-center">
            ...
            <p className="text-blue-200 mt-2 font-semibold underline">শিক্ষক প্রোফাইল ও পূর্ণাঙ্গ সার্ভিস ডাটাবেস ব্যবস্থাপনা</p>
          </div>

          <div className="p-4 md:p-8">
            <form onSubmit={handleSubmit} className={`p-6 md:p-10 rounded-3xl border-2 shadow-inner ${isEditing ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-100'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 ...
                 <input type="number" name="basicPay" value={formData.basicPay} placeholder="বর্তমান মূল বেতন" onChange={handleChange} className="p-3 border rounded-xl bg-white" />
              </div>
              <div className="flex gap-4 mt-8">
                  <button type="submit" className={`flex-grow py-4 rounded-2xl text-white font-bold shadow-xl uppercase transition-all ${isEditing ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-900 hover:bg-black'}`}>{isEditing ? "তথ্য আপডেট করুন" : "সংরক্ষণ করুন"}</button>
                  ...
                  {teachers.map((t) => (
                    <tr key={t._id} className="hover:bg-slate-50 transition border-b">
                      ...
                      <td className="p-5 flex flex-wrap justify-center gap-2">
                        <button onClick={() => setSelectedTeacher(t)} className="bg-blue-600 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow transition hover:bg-blue-800">বিস্তারিত</button>
                        <button onClick={() => startEdit(t)} className="bg-amber-500 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow transition hover:bg-amber-600">এডিট</button>
                        <button onClick={() => handleDelete(t._id)} className="bg-red-500 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow transition hover:bg-red-700">মুছুন</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {/* Footer */}
          <footer className="bg-slate-900 text-white p-8 border-t-4 border-blue-600 flex flex-col md:flex-row items-center gap-6">
      ...
      {/* --- সলিড হোয়াইট মডাল (কন্টেইনারের বাইরে রাখা হয়েছে ফিক্সের জন্য) --- */}
      {selectedTeacher && (
          ...
          <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-t-8 border-blue-900 relative z-[999999999]">
            ...
            <div className="p-8 md:p-12 space-y-8 bg-white">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 border-b pb-8">
                ...
                <div className="space-y-3">
                  <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1">২. পেশাগত ও শিক্ষা</h3>
                  <p><strong>পদবী:</strong> {selectedTeacher.designation}</p>
                  <p><strong>মূল বেতন:</strong> {selectedTeacher.basicPay || 0} ৳</p>
                  <p><strong>SSC তথ্য:</strong> {selectedTeacher.eduSSC || "নেই"}</p>
                  <p><strong>স্নাতক:</strong> {selectedTeacher.eduGrad || "নেই"}</p>
                </div>
              </div>
              <p className="bg-gray-50 p-4 rounded-xl italic"><strong>ঠিকানা:</strong> {selectedTeacher.presentAddr || selectedTeacher.address || "নেই"}</p>
            </div>
            <div className="p-6 bg-gray-100 flex justify-center">
              <button onClick={() => setSelectedTeacher(null)} className="bg-blue-900 text-white py-3 px-20 rounded-xl font-bold hover:bg-black transition-all">বন্ধ করুন</button>
            </div>
          </div>
        </div>
      )}
