...
const TeacherDatabase = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    eduSSC: '', eduGrad: '', firstJoinDate: '', prlDate: '', basicPay: '', bankAcc: '', bankName: '', routingNumber: '', incomeTax: '', ibasId: ''
  });
  ...
  const resetForm = () => {
    setFormData({ nameBn: '', nameEn: '', fatherName: '', motherName: '', presentAddr: '', permanentAddr: '', nid: '', designation: '', department: '', eduSSC: '', eduGrad: '', firstJoinDate: '', prlDate: '', basicPay: '', bankAcc: '', bankName: '', routingNumber: '', incomeTax: '', ibasId: '' });
    setIsEditing(false);
    setEditId(null);
  };
    ...
    <div className="min-h-screen bg-slate-50 p-2 md:p-6 font-sans text-gray-800">
      <div className="max-w-7xl mx-auto bg-white shadow-2xl rounded-3xl border-t-8 border-blue-900 overflow-hidden relative">
        ...
        <div className="p-4 md:p-8">
          ...
          <form onSubmit={handleSubmit} className={`p-6 md:p-10 rounded-3xl border-2 shadow-inner transition-all ${isEditing ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-100'}`}>
            ...
            <div className="flex gap-4 mt-8">
                <button type="submit" className={`flex-grow py-4 rounded-2xl text-white font-bold shadow-xl uppercase transition-all ${isEditing ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-900 hover:bg-black'}`}>
                    {isEditing ? "তথ্য আপডেট করুন" : "তথ্য সংরক্ষণ করুন"}
                </button>
                {isEditing && <button type="button" onClick={resetForm} className="bg-gray-500 text-white px-8 rounded-2xl font-bold">বাতিল</button>}
            </div>
          </form>
                ...
                {teachers.map((t) => (
                  <tr key={t._id} className="hover:bg-slate-50 transition border-b">
                    ...
                    <td className="p-5 flex flex-wrap justify-center gap-2">
                      <button onClick={() => setSelectedTeacher(t)} className="bg-blue-600 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow">বিস্তারিত</button>
                      <button onClick={() => startEdit(t)} className="bg-amber-500 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow">এডিট</button>
                      <button onClick={() => handleDelete(t._id)} className="bg-red-500 text-white px-4 py-2 rounded-full text-[10px] font-bold shadow">মুছুন</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <footer className="bg-slate-900 text-white p-8 mt-12 border-t-4 border-blue-600 flex flex-col md:flex-row items-center gap-6">
            <div style={{ width: '40px', height: '40px' }} className="rounded-full border-2 border-blue-400 overflow-hidden shadow-xl bg-white">
            ...
            <div>
               ...
               <p className="text-blue-400 text-xs font-bold">সেকশন অফিসার (সফটওয়্যার ইঞ্জিনিয়ার) | BBUASM</p>
            </div>
        </footer>

        {/* --- বিস্তারিত সলিড মডাল (FIXED MODAL) --- */}
        {selectedTeacher && (
           <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 9999999, position: 'fixed', top: 0, left: 0, width: '100%', height: '100%' }}>
              <div className="absolute inset-0 bg-black bg-opacity-95 backdrop-blur-md" onClick={() => setSelectedTeacher(null)}></div>
              
              <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-t-8 border-blue-900 relative z-[10000000]">
                  ...
                  <div className="p-8 md:p-10 space-y-8 bg-white">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-b pb-8 text-base">
                      <div className="space-y-4">
                        <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1">১. ব্যক্তিগত তথ্য</h3>
                        <p><strong>নাম:</strong> {selectedTeacher.nameBn || selectedTeacher.nameBN}</p>
                      ...
                      <div className="space-y-4">
                        <h3 className="font-bold text-blue-900 uppercase text-xs border-b pb-1">২. পেশাগত তথ্য</h3>
                        ...
                        <p><strong>যোগদান:</strong> {selectedTeacher.firstJoinDate || "নেই"}</p>
                      </div>
                    </div>
                    <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 text-blue-900">
                        <p><strong>শিক্ষাগত যোগ্যতা (SSC/স্নাতক):</strong> <br/> {selectedTeacher.eduSSC || selectedTeacher.eduGrad || "প্রদত্ত নয়"}</p>
                        <p className="mt-4"><strong>বর্তমান ঠিকানা:</strong> <br/> {selectedTeacher.presentAddr || selectedTeacher.address || "প্রদত্ত নয়"}</p>
                    </div>
                  </div>

                  <div className="p-6 bg-gray-100 flex justify-center">
                    <button onClick={() => setSelectedTeacher(null)} className="bg-blue-900 text-white py-3 px-16 rounded-2xl font-bold shadow-lg hover:bg-black transition-all uppercase tracking-widest">বন্ধ করুন</button>
                  </div>
              </div>
           </div>
        )}
